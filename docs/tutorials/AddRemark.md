...
/**
 * Changes the remark of an existing stall in the address book.
 */
...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD
            + ": Edits the remark of the stall identified "
            + "by the index number used in the last stall listing. "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the stall in the filtered stall list to edit the remark
     * @param remark of the stall to be updated to
     */
...
Now that we have all the information that we need, let’s lay the groundwork for propagating the remarks added into the in-memory storage of stall data. We achieve that by working with the `Person` model. Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the stall’s name). That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a stall.

### Add a new `Remark` class

Create a new `Remark` in `seedu.address.model.stall`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each stall.

Simply add the following to [`seedu.address.ui.StallCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-639834f1e05afe2276a86372adf0fe5f69314642c2d93cfa543d614ce5a76688).
...
``` java
public PersonCard(Person stall, int displayedIndex) {
    //...
    remark.setText(stall.getRemark().value);
}
```
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person stallToEdit = lastShownList.get(index.getZeroBased());
        Person editedStall = new Person(
                stallToEdit.getName(), stallToEdit.getPhone(), stallToEdit.getEmail(),
                stallToEdit.getAddress(), remark, stallToEdit.getTags());

        model.setPerson(stallToEdit, editedStall);
        model.updateFilteredPersonList(PREDICATE_SHOW_ALL_PERSONS);

        return new CommandResult(generateSuccessMessage(editedStall));
    }
     ...
     * {@code stallToEdit}.
     */
    private String generateSuccessMessage(Person stallToEdit) {
        String message = !remark.value.isEmpty() ? MESSAGE_ADD_REMARK_SUCCESS : MESSAGE_DELETE_REMARK_SUCCESS;
        return String.format(message, stallToEdit);
    }
```
